...
Presenting a plugin for Coq, based on the definitional UIP extension,
providing a brand new `Lean Import` command.

This is an experimental alpha.

The `Lean Import` command itself is implemented as a plugin, but note that the typechecking of Lean terms requires UIP.
